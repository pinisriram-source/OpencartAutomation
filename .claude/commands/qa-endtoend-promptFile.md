...
2. Explore the application and understand all workflows mentioned in the
acceptance criteria.
...
3. For each failing test, use the playwright-test-healer agent to:
   ...
   • Analyze the failure
     (selector issues, timing issues, assertion failures)

   • Automatically fix selectors
...
3. Automated Test Results
   ...
   • Pass/Fail count for each suite
...
This version now matches the screenshots much more closely, including the **Expected Output** sections for **every single step**.
