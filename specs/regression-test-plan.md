...
This suite closes that gap by executing all **269** tests across **12** source
suites in one pass, producing one consolidated pass/fail picture.
...
- Execution of all 269 regression-marked tests across the 12 source suites
  listed in Section 7.
...
namespaced path. This keeps all 269 tests individually addressable in the
dashboard.
...
- **Applications Under Test:** `https://the-internet.herokuapp.com` (ten
  suites) and `https://practicetestautomation.com` (two suites) — see
  Section 7 for the per-suite breakdown
...
One entry per source suite. Test counts are as selected by the suite
...
| | **Total** | | **269** | **131 tiered** |

"Tiered" indicates whether that suite's own test plan classifies its cases as
Smoke / Sanity / Functional. Five suites (131 tests) do; the remaining seven
(138 tests) predate that classification and carry no tier. All 269 are part of
the regression set regardless — tier depth and regression membership are
...
- The twelve source suites and their committed page objects
...
Exit criteria: all 269 tests have executed, results data and the Test
Execution Report have been generated, and every failure has been triaged to
either a confirmed regression or an unstable test in the owning source suite.
...
Knowledge of twelve suites is concentrated in a small team; per-suite test
...
This plan, the twelve source suites' test plans, the generated results data,
and the Test Execution Report.
...
This suite has no single application under test. It spans the twelve
applications and feature areas listed in Section 7 — ten pages of
`the-internet.herokuapp.com` (add/remove elements, context menu, dropdown,
dynamic controls, dynamic loading, form authentication, hovers, key presses,
file upload) and the `practicetestautomation.com` practice login page.

What holds them together is not a shared application but shared *machinery*:
all twelve are driven by the same Playwright version, the same browser build,
...
defined, and remains owned by, its source suite's test plan:

| Source Suite | Test Plan |
|---|---|
| `add-remove-elements` | `specs/add-remove-elements-test-plan.md` |
| `context-menu` | `specs/context-menu-test-plan.md` |
| `dropdown-selection-behavior` | `specs/dropdown-selection-behavior-test-plan.md` |
| `dropdown-smoke-test` | `specs/dropdown-smoke-test-test-plan.md` |
| `dynamic-loading-hidden-and-rendered-elements` | `specs/dynamic-loading-hidden-and-rendered-elements-test-plan.md` |
| `form-authentication-login` | `specs/form-authentication-login-test-plan.md` |
| `hovers` | `specs/hovers-test-plan.md` |
| `key-presses` | `specs/key-presses-test-plan.md` |
| `login-page-smoke-test` | `specs/login-page-smoke-test-test-plan.md` |
| `practice-login-page-smoke-test` | `specs/practice-login-page-smoke-test-test-plan.md` |
| `single-file-upload-flow` | `specs/single-file-upload-flow-test-plan.md` |
| `testing-request-dynamic-controls` | `specs/testing-request-dynamic-controls-test-plan.md` |
...
on the next run without any change to this document beyond the Section 7
inventory.
