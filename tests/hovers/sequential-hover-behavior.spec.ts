...
test.describe('Sequential Hover Behavior', () => {
  ...
  test('TC-HOVERS-009: Hovering from first to second avatar hides first caption and shows second', { tag: ['@functional', '@regression'] }, async ({ page }) => {
  ...
  test('TC-HOVERS-010: Hovering from second to third avatar hides second caption and shows third', { tag: ['@functional', '@regression'] }, async ({ page }) => {
    // 1. Navigate to the Hovers page
    // expect: Page loads successfully
    await expect(page).toHaveURL('https://the-internet.herokuapp.com/hovers');
    await expect(page).toHaveTitle('The Internet');
  ...
  test('TC-HOVERS-011: Hovering from third to first avatar hides third caption and shows first', { tag: ['@functional', '@regression'] }, async ({ page }) => {
    // 1. Navigate to the Hovers page
    // expect: Page loads successfully
    await expect(page).toHaveURL('https://the-internet.herokuapp.com/hovers');
    await expect(page).toHaveTitle('The Internet');
  ...
  test('TC-HOVERS-012: Hovering all three avatars in sequence shows only one caption at a time', { tag: ['@sanity', '@regression'] }, async ({ page }) => {
    // 1. Navigate to the Hovers page
    // expect: Page loads successfully
    await expect(page).toHaveURL('https://the-internet.herokuapp.com/hovers');
    await expect(page).toHaveTitle('The Internet');
