...
test.describe('No Double-Start Protection', () => {
  test('TC-DYNLOAD-020: Example 1 Start button is not clickable during loading', { tag: ['@sanity', '@regression'] }, async ({ page }) => {
  ...
  test('TC-DYNLOAD-021: Example 2 Start button is not clickable during loading', { tag: ['@functional', '@regression'] }, async ({ page }) => {
  ...
  test('TC-DYNLOAD-022: Example 1 rapid clicks on Start button only trigger loading once', { tag: ['@functional', '@regression'] }, async ({ page }) => {
    const examplePage = new DynamicLoadingExamplePage(page, 1);
    await examplePage.navigate();
    await expect(examplePage.startButton).toBeVisible();
  ...
  test('TC-DYNLOAD-023: Example 2 rapid clicks on Start button only trigger loading once', { tag: ['@functional', '@regression'] }, async ({ page }) => {
    const examplePage = new DynamicLoadingExamplePage(page, 2);
    await examplePage.navigate();
    await expect(examplePage.startButton).toBeVisible();
