...
Most suites own spec files under `tests/<slug>/`. An **aggregator suite** owns
none: it is defined by a `tests/<slug>/suite.config.ts` that *selects* tests
already living in other suites. `tests/regression/` is the first and currently
only one — it selects every `@regression`-tagged test across all pipeline
suites (269 tests, 12 source suites) so the whole cross-suite regression set
runs as one pipeline suite with one consolidated report.

The point is that a test appears there because a source suite already defines
it — never because it was copied. The source suite remains the owner: a
failure surfaced by the aggregator is fixed in the source suite, and coverage
is added by adding it to the source suite, not here. Membership is automatic —
tag a test `@regression` in any suite and the next aggregator run picks it up
with no edit to the aggregator.

Run it locally with either of:

```
npx playwright test --config=tests/regression/suite.config.ts --project=chromium
npx playwright test --project=chromium --grep @regression
```

**The config must use absolute paths.** Playwright resolves relative config
paths against the config file's own directory, so inheriting the base config's
`testDir: './tests'` and `'./reporters/screenshot-collector.ts'` would resolve
them under `tests/regression/` and silently find nothing. `suite.config.ts`
spreads `playwright.config.ts` and then overrides `testDir`, `outputDir`,
`reporter`, and `projects` with `path.join(REPO_ROOT, ...)` values.

Three things in the pipeline branch on `tests/<slug>/suite.config.ts` existing,
all of them generic rather than hardcoded to the `regression` slug:

- **`pipeline-automation.yml`** skips generation entirely (there is nothing to
  generate — generating would author the very duplicate spec files the suite
  exists to avoid), and its "Verify automation suite was created" check accepts
  a `suite.config.ts` in place of spec files. The tier gate is skipped too,
  since it only runs on fresh generations.
- **`pipeline-execute.yml`** runs `--config=tests/<slug>/suite.config.ts`
  instead of passing `tests/<slug>` as a path argument — a path argument can't
  express "tests from many directories".
- **`pipeline-execute.yml`'s results-JSON prompt** namespaces test IDs (below).

**TC-IDs are only unique within a suite**, which matters the moment tests are
pooled: `TC-LOGIN-001` through `TC-LOGIN-016` exist in *both*
`login-page-smoke-test` and `practice-login-page-smoke-test`, and
`TC-DROPDOWN-003`/`004`/`005`/`006`/`010`-`014` in *both* dropdown suites. So
in an aggregator suite:

- Each test's `id` in `streamlit_app/data/<slug>-test-results.json` is
  `<source-suite>/<TC-ID>` (e.g. `hovers/TC-HOVERS-001`), and `meta.suite_path`
  is `tests/`. The `suites` array gets one entry per source suite, with each
  test's `use_case` pointing at its source suite's `UC<n>`.
- `reporters/screenshot-collector.ts` nests one level deeper —
  `reports/screenshots/<slug>/<source-suite>/<TC-ID>.png` — whenever the run's
  `SLUG` differs from the test's own folder under `tests/`. Without this, each
  colliding pair silently overwrites the other. When `SLUG` equals the folder
  (every normal suite) the path is unchanged, so existing suites are
  unaffected.
- These two line up on purpose: the dashboard builds the screenshot path as
  `reports/screenshots/<slug>/<id>.png`, which with a namespaced `id` resolves
  to exactly the nested path the collector writes — no dashboard change needed
  for screenshots.
- The dashboard's Test Case Detail tab does need to split a namespaced id to
  find the source, since `find_test_block` searches by bare TC-ID and would
  otherwise match whichever colliding suite sorted first.

An aggregator's test plan follows the same 16-section structure as every other
plan, but its **Section 18 defines no scenarios** — it points at the source
suites' plans instead, because there is nothing here for the generator to
consume. Worked example: `specs/regression-test-plan.md`.

**Section 7's inventory is generated, not hand-written.** Membership is
dynamic, so a hand-maintained table of source suites and counts goes stale the
moment a new suite is generated, with nothing to catch it.
`.github/scripts/sync-regression-inventory.js <slug>` asks Playwright what the
config actually selects (`--list --reporter=json`), derives each source suite's
test count, tiered status, and target URL, and rewrites the block between the
`<!-- BEGIN:REGRESSION-INVENTORY -->` / `<!-- END:REGRESSION-INVENTORY -->`
markers. `pipeline-execute.yml` runs it on every aggregator execution and
commits the refreshed plan alongside that run's results.

Because of that, **Section 7 is the only place in an aggregator's plan allowed
to state counts** — every other section refers to it rather than repeating a
number, so one regenerated block keeps the whole document true. For the same
reason Section 18 states the `specs/<source-suite>-test-plan.md` naming rule
instead of enumerating a second table that could disagree with Section 7.
