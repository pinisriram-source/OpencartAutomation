...
7-section strategy overview before the scenario-by-scenario content, in this
fixed order:

1. **Project Overview** — `Project Name` / `Objective` / `Target Audience`
2. **Scope of Automation** — `In-Scope` / `Out-of-Scope` bullets
3. **Test Strategy & Framework** — `Automation Tools` / `Test Architecture` /
   `Test Data Strategy`
4. **Test Environment** — `Application Under Test` / `Browser Matrix` /
   `CI/CD Integration`
5. **Execution & Schedule** — `Execution Frequency` / `Milestones`
6. **Entry and Exit Criteria** — `Entry Criteria` / `Exit Criteria`
7. **Roles and Responsibilities** — one line each for QA Automation Engineer
   / DevOps-CI Engineer / Product Owner-QA Lead

Followed by the plan's existing content, renumbered as:

8. **Application Overview** — the free-text feature description that used to
   open the file
9. **Detailed Test Scenarios** — the `### <feature-group>` groups and
   `#### <feature-group>.<scenario>. TC-<MODULE>-<NNN>: <Title>` scenarios,
   unchanged (this section is what actually drives the automation generator,
   the `**Tier:**` tags, and the CI tier-gate — sections 1-7 are strategy
   context for human reviewers, not machine-read).

Sections 3-7 are mostly constant across suites (this pipeline always uses
Playwright/TypeScript, POM, GitHub Actions, etc.) — fill them from the
project's actual setup rather than inventing per-suite values. Sections 1-2
and the `Application Under Test` line in section 4 are suite-specific:
derive `Objective` and `Scope` from the submitted acceptance criteria.

Worked example: `specs/hovers-test-plan.md`. Same scope-of-adoption caveat
